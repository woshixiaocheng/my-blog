<template>
    ...
    <el-affix position="top" taget="body">
      <transition name="el-fade-in-linear">
        <div v-show="show">
          ...
          <el-menu v-if="fullScreen" :default-active="activeIndex"
            :class="{ whiteMenu: whiteMenu, menu: true, webMenu: true }" mode="horizontal" :ellipsis="false" router>
          ...
          <el-menu v-else :default-active="activeIndex" :class="{ whiteMenu: whiteMenu, menu: true }" v-show="show"
 ...
 <component :key="route.name" :is="Component" v-else />
  ...
  <el-drawer v-model="drawer" :show-close="false" size="60%" direction="ltr" class="drawer">
    ...
    <el-menu :default-active="activeIndex" mode="vertical" :ellipsis="false" router class="mobileMenu menu">
      <el-menu-item index="/">首页</el-menu-item>
      <el-menu-item index="/classify">归档</el-menu-item>
      <el-menu-item index="1" :route="{ path: '/article', query: { sort_id: 1 } }">技术文章</el-menu-item>
      <el-menu-item index="2" :route="{ path: '/article', query: { sort_id: 2 } }">生活随笔</el-menu-item>
