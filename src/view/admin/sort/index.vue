...
const changePage=(newPage)=>{
  page.value.page=newPage
    ...
    const showSortsByPage=async()=>{
      const data =await getSortsByPage({page:page.value.page,size:page.value.size})
      list.value=data.data
      const data1=await getSorts()
    ...
    const deleteSort=(id)=>{
      ElMessageBox.confirm('确认删除？',{ confirmButtonText: '确定',
          cancelButtonText: '取消',}).then(async()=>{
           const {data}= await delSort({id:id})
          if(data&&data.code===0){
            ElMessage({
              message:'不能删除该数据',
              type:'error'
    ...
    return {showDialog,list,page,total,changePage,deleteSort,addDialog,editDialog,sort,showSortsByPage}
  }
}
</script>
